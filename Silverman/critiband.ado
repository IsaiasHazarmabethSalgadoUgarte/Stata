*! version 1.00 28/02/2007       Stata Journal xx: xxxx
program define critiband
  version 4.0
*First written 28/02/2007; Last revision 09/03/2007 
*Author: Salgado-Ugarte Isaias H.
*Based on the idea of Rodriguez-Rojas, José R.
*This program calculates the kde's and count the modes in order to
*find the critical bandwidths for use with the Silverman multimodality
*smoothed bootstrapped test. As in the silvtest.ado program, 
...
local options 
 "BWHigh(real 0) BWLow(real 0) STsize(real 0) Mval(integer 0)
...
*  Mode counter
   ...
   local nomo= `sumo'[_N]
   #delimit ;
   noi display "Estimation number = " _col(15)`countr' _col(30)"Bandwidth = " `hv'
