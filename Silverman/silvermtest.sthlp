...
This set of (updated) Stata programs allows to investigate the modality of a
quantitative variable by means of the Silverman (1981) smoothed bootstrap 
multimodality test.
...
As in the previous {hi:silvtest.ado} program, to estimate the KDE it uses the 
WARPing procedure based on the algorithms described in Härdle (1991), Scott 
(1992; 2015), Salgado-Ugarte & Saito-Quezada (2020). The program produces 
(optionally) density graphs and list in the {hi:Results} window the bandwidths 
with their corresponding number of modes in the specified bandwidths range. 
...
{pstd}To B. Silverman, D.W. Scott and W. Härdle, for having provided the basis 
for our algorithms.
