{smcl}
{* *! version 2.0.0 09/08/2020; revised 15/03/25}{...}
{hline}
...
{pstd}{hi:xvar}, {hi:criticalbw}, and {hi:sdx}, are not optional. These values are required by the 
{cmd:boot} command. If the user does not provide them, the program halts and displays an error message on screen.

{pstd}This program adjust (smooth) the bootstrap samples generated by Stata's {cmd:boot} command according to Silverman (1981) to perform his propossed multimodality test.

{pstd}The program must be applied sequentially to each of the several number of modes to be tested.
