...
{p2colset 5 18 20 2}{...}
{p2col :{hi:nuamodes} {hline 2}} Counting and estimation of antimodes in density/frequency data (Updated version){p_end}
{p2colreset}{...}

{title:Syntax}
{p 8 17 2}
{cmd:nuamodes denvar midvar} {ifin} [{cmd:,} {cmdab:amo:des(}{it:#}{cmd:)}]

{title:Description}

{pstd}{cmd:nuamodes} calculates the number of antimodes of a density estimation or a
frequency distribution and if especified lists their estimated values. The user
must include the variable with the density or frequency values ({hi:denvar}) and the
corresponding midpoints ({hi:midvar}). Useful for histogram or kernel density 
estimation antimodes determination.

{title:Options}

{phang}{opt {hi:amo}des} lists the estimated values for each antimode.
...
{pstd}Generating density and midpoint values with aprox. 1/2 of "optimal" 
Gaussian bw

{phang}{stata "nuamodes den5 mid5, amodes" :. nuamodes den5 mid5, amodes}{p_end}

{pstd}Will display the number of antimodes and their estimated values. 
...
{phang}On-line: {hi:help} for {help numodes}, {help warpdenm1}, {help kerneld}, {help bandw1}, {help l2cvwarpy}, {help bcvwarpy}
{p_end}