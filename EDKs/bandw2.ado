program bandw2, rclass
 *Authors: Salgado-Ugarte, IH, M Shimizu, T Taniuchi & VM Saito-Quezada
 *Based on bandw.ado; first written: 94/10/24 (V 1.0); revised: 18/06/2025
 *30/03/2026
 version 11.0
 ...
 display as text "Sturges' number of bins = " as res _col(50) %8.0f int(ceil(`ks'))
 display as text "Dixon & Kronmal number of lines = " as res _col(50) %8.0f int(ceil(`st'))
 display as text "Oversmoothed number of bins <= " as res _col(50)%8.0f int(floor(`osb'))
 display as text _dup(60) "-"
 display as text "FP oversmoothed number of bins <= " as res _col(50)%8.0f int(floor(`fposb'))
 display as text _dup(60) "="
 display as text "Scott's optimal Gaussian binwidth = " as res _col(50)%8.4f `hs'
 ...
 display as text _dup(60) "_"
 ret scalar nbin_ks = `ks'
 ret scalar nbin_st = `st'
