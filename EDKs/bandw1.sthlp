...
{hline}
{cmd:help bandw1}                                              [STB-27: snp6.2]
{hline}

{title:Title}

{p2colset 5 18 20 2}{...}
{p2col :{hi:bandw1} {hline 2}}Bandwidth selection rules for univariate density estimators (Updated version){p_end}
{p2colreset}{...}

{title:Syntax}
{p 8 17 2}
{cmd:bandw1} {varname} {ifin} {cmd:,} {cmdab:k:code(}{it:#}{cmd:)}

{title:Description}

{pstd}{cmd:bandw1} calculates several data-based number of bins and binwidths-
...
in a table. This is the updated version of {hi:bandw}
...
I Rules for number of bins
  a) Histograms
    1) Sturges' number of bins rule:
	 k = 1 + log2(n)
    2) OS number of bins rule: (b = max; a = min)
	 (b - a)/h* >= (b - a) / hOS = (2n)^(1/3)
  b) Frequency polygons
    3) FP OS Number of bins rule:
	 (b - a)/h* >= ((147/2)*n)^(1/5)
II Rules for histogram bindwidth selection
    4) Scott's Normal bindwidth reference rule:
	 h = 3.5*sigma*n^(-1/3)
    5) Freedman-Diaconis robust binwidth rule:
	 h = 2(IQR)n^(-1/3)
    6) Oversmoothed (OS) binwidth:
	 h* <= (b - a) / (2n)^(1/3) = hOS
    7) OS homoscedastic rule:
	 h*<= 3.729*sigma*n^(-1/3) = hOS
    8) OS robust rule:
	 h* <= 2.603(IQR)n^(-1/3) = hOS
III Rules for FP binwidth selection
    9) FP Gaussian reference rule:
	 h = 2.15*sigma*n^(-1/5)
   10) FP OS rule:
      h <= (23,328/343)^(1/5) sigma*n^(-1/5) = 2.33*sigma*n^(-1/5) = hOS
IV Rules for kernel bandwidth selection (Gaussian kernel by default):
   11) Silverman's Normal bandwidth reference rule:
	 ho=0.9 min(sigma,IQR/1.349)n^(-1/5)
   12) Haerdle's Better rule of thumb:
	 ho = 1.06 min(sigma, IQR/1.349)n^(-1/5)
   13) Scott's (Gaussian) kernel oversmoothed bandwidth:
...
{psee}
    STB: snp6 (STB-16); snp6.1 (STB-26); snp6.2 (STB-27); snp6.13 (STB-38)
