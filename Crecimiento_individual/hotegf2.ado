...
program define hotegf2, eclass
	version 7.0
*Author: Salgado-Ugarte, I.H. & V.M. Saito-Quezada
*First written: 24/04/2002 (version 1.00); revised 03/05/2002;
*Last revised 21/11/2003; 08/11/2021
...
*38; 223-236).
*This is the version with the Bonferroni inequalities for calculating the
*confidence intervas according to Headrick, M.R. & F.J. Margraf Jr. 1982, 
*Comment on Multivariate Analysis as a Means of Comparing Growth in Fish.
*Canadian Journal of Fisheries and Aquatic Sciences, 39(3): 529-530.
...
di as text "Multivariate test for Growth Function comparison of two"
di as text "populations; based on Bernard (1981) and Headrick & Margraf (1982)"
	di _dup(78) "_"
...
*	local sl = (100 - `cl')/100;	
	...
	local T0 = `T2t'^.5;
	*local sl = (100-`cl')/100;	
    local t0 =  abs(invt(`dof',`sl'/(2*3)));
	
*	if `t0' < `T0' 

	*local llLi = `Dp'[1,1] - (((`n1'+`n2')/(`n1'*`n2'))*`S'[1,1]^.5)^.5;
    local llLi = `Dp'[1,1] - (((`n1'+`n2')/(`n1'*`n2'))*`S'[1,1]^.5)^.5;
	local ulLi = `Dp'[1,1] + (((`n1'+`n2')/(`n1'*`n2'))*`S'[1,1]^.5)^.5;
	local FcritLi= (`n1'*`n2'*(`n1'+`n2'-4)*(`Dp'[1,1])^2)/(3*(`n1'+`n2')*
		(`n1'+`n2'-2)*`S'[1,1]);

	local llk = `Dp'[2,1]- (((`n1'+`n2')/(`n1'*`n2'))*`S'[2,2]^.5)^.5; 
	local ulk = `Dp'[2,1]+ (((`n1'+`n2')/(`n1'*`n2'))*`S'[2,2]^.5)^.5;;
	local Fcritk= (`n1'*`n2'*(`n1'+`n2'-4)*(`Dp'[2,1])^2)/(3*(`n1'+`n2')*
		(`n1'+`n2'-2)*`S'[2,2]);

	local llto = `Dp'[3,1]- (((`n1'+`n2')/(`n1'*`n2'))*`S'[3,3]^.5)^.5;;
	local ulto = `Dp'[3,1]+ (((`n1'+`n2')/(`n1'*`n2'))*`S'[3,3]^.5)^.5;;
	...
	local con=`cl';
	di as result _newline "{text}  Bonferroni Confidence intervals of " %2.0f `con'      "{text} % " _column(56) "{text}Critical F";


	di as result _newline %10.4f `llLi' "{text} <= L_inf1 - L_inf2 <=" 
	_column(30) %10.4f 	`ulLi' _column(55) %10.4f `FcritLi';
